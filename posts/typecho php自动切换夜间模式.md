---
titles: typecho php自动切换夜间模式
date: 2020-03-10 17:34:19
---

typecho用php直接后台自动切换夜间模式
准备一套darkmode的css即可
...
<?php
  ...
  $hour = date('H');
  //echo '<!--',$hour,'-->';   //调试
  if($hour > 7 && $hour <= 18){   //7-18点
  $this->options->themeUrl('style.css');   
  }
  else{
  $this->options->themeUrl('darkstyle.css');   
  }
?>
">
```

default修改来的的darkstyle.css
...
@media(-webkit-min-device-pixel-ratio:2),(min-resolution:192dpi) {
  #search button{background-image: url(img/icon-search@2x.png);
